...
let car = inventory.filter(function(val, i, arr) {
  return val.id === 33;
});
...
inventory.forEach(function(val, i, arr) {
  carModels.push(val.car_model);
});
...
let oldCar = carYears.filter(function(val, i, arr) {
  return val < 2000;
});
...
let BMWAndAudi = inventory.filter(function(val, i, arr) {
